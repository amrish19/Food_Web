...
function Order() {
    ...
    const res = id.split('&').reduce((acc, param) => {       // "res"  Contain response from preview page image id and email of restutant 
    ...
    const fun = async (Email) => { 
      ...
      <div className='ml-10'>
        ...
        content = <div className='ml-10'><div className='m-auto  animate-spin rounded-full  h-20 w-20 border-t-8  border-b-8 border-red-400'></div> <h3 className='text-xl font-bold text-gray-500 mt-5'>Processing Your Order</h3> </div>;
      }
  ...
  return (
   ...
   <div className=' mt-20 ml-20  inline-flex'>
    <img  width="40%" height="40%"  src={storage.getFilePreview("664786d100198bc9a452",res.img_id)} alt="" />

    
     

    <button  onClick={()=>fun(res.Email)} className=' h-10 w-30  ml-24 px-6 py-2 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-red-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 transform hover:-translate-y-1 hover:scale-105 transition duration-300 ease-in-out'> Order now </button>
    
    <button
           
            className=" h-10 w-30 ml-10 flex items-center px-6 py-2 bg-green-400 text-white font-bold rounded-full shadow-md hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-75 transform hover:-translate-y-1 hover:scale-105 transition duration-300 ease-in-out"
        >
            <svg
